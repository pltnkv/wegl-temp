...
(function(PrimitiveType) {
    PrimitiveType[PrimitiveType["TRIANGLE_WITH_COLOR"] = 1] = "TRIANGLE_WITH_COLOR";
    PrimitiveType[PrimitiveType["IMAGE"] = 2] = "IMAGE";
    PrimitiveType[PrimitiveType["RENDER_TO_TEXTURE"] = 3] = "RENDER_TO_TEXTURE";
...
const STRIDE_IN_FLOATS = MONSTER_SHADER_ATTRIBUTES.reduce((acum, entry)=>acum + entry.size
, 0);
...
const buffers = []// vertex buffer + index buffer goes here
;
let debugTextureWithoutAtlasesMemoryCount = 0;
let atlasQuads;
let atlasQuadsDestUVs;
let atlasQuadsDestUVsBuffer;
let atlasQuadsIndices;
// fps output
...
function setupAtlasRenderer() {
    const atlasQuadsBuffer = new Float32Array([
        PrimitiveType.RENDER_TO_TEXTURE,
        1,
        0,
        0,
        PrimitiveType.RENDER_TO_TEXTURE,
    ...
    gl.bufferData(gl.ARRAY_BUFFER, 256 * Float32Array.BYTES_PER_ELEMENT, gl.DYNAMIC_DRAW);
    atlasQuadsDestUVsBuffer = new Float32Array(256);
    atlasQuadsIndices = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, atlasQuadsIndices);
    const indices = new Uint16Array(96);
    let j = 0;
...
function setupShaders() {
    monsterShader = compileShader("monster", [
        "view",
        'textures',
        'views'
    ], [
        'texId'
    ].concat(MONSTER_SHADER_ATTRIBUTES.map((entry)=>entry.name
    )));
    monsterShader.use();
    gl.uniform1iv(monsterShader.textures, [
        0,
        1,
        2,
        3,
...
function getTextSizeInPixels(text, font) {
...
function renderText(text, font, color, size) {
    textContext.font = font;
...
async function fillBuffer() {
    ...
    for(let i = 0; i < 30000; i++)shapes.push({
    ...
    const promises = [];
    for(let i3 = 1; i3 <= 1000; i3++)promises.push(loadImage(`images/${i3}.jpg`));
    const images = await Promise.all(promises);
    const combination = [].concat(texts, images, shapes, lines);
    for(let i4 = combination.length - 1; i4 > 0; i4--){
        const j = Math.floor(Math.random() * (i4 + 1));
        const temp = combination[i4];
        combination[i4] = combination[j];
        combination[j] = temp;
    }
    const randomCombination = combination;
    const positions = getElementsPositionsWithoutIntersections(randomCombination);
    for(let i5 = 0; i5 < randomCombination.length; i5++){
        const element = randomCombination[i5];
        const pos = positions[i5];
        if (element.line) addLine([
            pos.x,
            pos.y,
            pos.x + element.width,
            pos.y + element.height
        ], element.line.width, element.line.width, element.color.r, element.color.g, element.color.b, element.color.a);
        else if (element.shape) addShape(SHAPE_STAR, pos.x, pos.y, element.color.r, element.color.g, element.color.b, element.color.a);
        else addImage(pos.x, pos.y, element.width, element.height, element.texture || element);
    }
}
function outputMemoryAndFPS(timestamp) {
    let memory = 0;
    for (const buffer of buffers)memory += buffer.atlases.length * (ATLAS_SIZE * ATLAS_SIZE * 4);
    const wastedPercent = memory > 0 ? Math.round((1 - debugTextureWithoutAtlasesMemoryCount / memory) * 100) : 0;
    const atlasMB = Math.round(memory / 1048576);
...
function addPrimitiveVertices(primitiveType, data, strideInFloats, indices, image, uvIndexes) {
    const strideInFloatsWithType = strideInFloats + 1;
    if (strideInFloatsWithType > STRIDE_IN_FLOATS) throw new Error("Not enough attribute slots.");
    if (data.length % strideInFloats !== 0) throw new Error("Data is not aligned with stride.");
    ...
    for(let i = 0; i < indices.length; i++)indexData[i] = buffer.nextVertexIndex + indices[i];
    const texIdData = new Float32Array(vertexCount);
    if (image) {
        if (!uvIndexes) throw new Error("Texture must include UV.");
        ...
        // ultra hack
        if (currentAtlasGroup !== prevAtlasGroup) buffer.drawCalls.push({
            indexCount: 0,
            textures: []
        });
    ...
    for(let i1 = 0; i1 < vertexCount; i1++){
        const srcIndex = i1 * strideInFloats;
        const dstIndex = i1 * STRIDE_IN_FLOATS;
        vertexData[dstIndex + 0] = primitiveType;
        for(let j = 0; j < strideInFloats; j++)vertexData[dstIndex + 1 + j] = data[srcIndex + j];
    }
    ...
    gl.bufferSubData(gl.ELEMENT_ARRAY_BUFFER, buffer.nextIndexIndex * Uint32Array.BYTES_PER_ELEMENT, indexData);
...
function addImage(x, y, width, height, image) {
...
function addLine(coordinates, lineWidthInside, lineWidthOutside, r, g, b, a) {
    if (coordinates.length % 2 !== 0) throw new Error('Invalid coordinates.');
    const pointsCount = coordinates.length / 2;
    if (pointsCount < 2) throw new Error("Line should consist of two or more points.");
    const vertices = [];
    const indices = [];
    let startIndex = 0;
    for(let i = 0; i < coordinates.length - 2; i += 2){
        ...
        const nx = -dy / len;
        const ny = dx / len;
        vertices.push(x0 + nx * lineWidthOutside, y0 + ny * lineWidthOutside, 0, r, g, b, a, x0 - nx * lineWidthInside, y0 - ny * lineWidthInside, 0, r, g, b, a, x1 + nx * lineWidthOutside, y1 + ny * lineWidthOutside, 0, r, g, b, a, x1 - nx * lineWidthInside, y1 - ny * lineWidthInside, 0, r, g, b, a);
        indices.push(startIndex + 0, startIndex + 1, startIndex + 2, startIndex + 1, startIndex + 2, startIndex + 3);
        startIndex += 4;
    }
    addPrimitiveVertices(PrimitiveType.TRIANGLE_WITH_COLOR, vertices, 7, indices);
}
function addShape(points, x, y, r, g, b, a) {
...
requestAnimationFrame(draw);
function describeVertices(vertexBuffer, texIdBuffer) {
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    let offset = 0;
    for (const attributeDesc of MONSTER_SHADER_ATTRIBUTES){
        const attributeIndex = monsterShader[attributeDesc.name];
        gl.vertexAttribPointer(attributeIndex, attributeDesc.size, gl.FLOAT, false, STRIDE_IN_BYTES, offset * Float32Array.BYTES_PER_ELEMENT);
        gl.enableVertexAttribArray(attributeIndex);
        offset += attributeDesc.size;
    }
    gl.bindBuffer(gl.ARRAY_BUFFER, texIdBuffer);
    gl.vertexAttribPointer(monsterShader.texId, 1, gl.FLOAT, false, Float32Array.BYTES_PER_ELEMENT, 0);
    gl.enableVertexAttribArray(monsterShader.texId);
}
function describeVerticesForAtlas(quadsBuffer, quadsDestUVsBuffer) {
    for (const attributeDesc of MONSTER_SHADER_ATTRIBUTES){
        const attributeIndex = monsterShader[attributeDesc.name];
    ...
    gl.vertexAttribPointer(monsterShader.slot1, 4, gl.FLOAT, false, 4 * Float32Array.BYTES_PER_ELEMENT, 0 * Float32Array.BYTES_PER_ELEMENT);
    gl.enableVertexAttribArray(monsterShader.slot1);
    gl.bindBuffer(gl.ARRAY_BUFFER, quadsDestUVsBuffer);
    gl.vertexAttribPointer(monsterShader.slot2, 4, gl.FLOAT, false, 4 * Float32Array.BYTES_PER_ELEMENT, 0 * Float32Array.BYTES_PER_ELEMENT);
    gl.enableVertexAttribArray(monsterShader.slot2);
}
function allocBuffer() {
    const vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, MAX_VERTICES * STRIDE_IN_BYTES, gl.STREAM_DRAW);
    const vertexBufferRAM = new Float32Array(MAX_VERTICES * STRIDE_IN_FLOATS);
    const texIdBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, texIdBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, MAX_VERTICES * Float32Array.BYTES_PER_ELEMENT, gl.STREAM_DRAW);
    const texIdBufferRAM = new Float32Array(MAX_VERTICES);
    const indexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBuffer);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, MAX_INDICES * Uint32Array.BYTES_PER_ELEMENT, gl.STREAM_DRAW);
...
function getBuffer(requiredVertices, requiredIndices) {
    const lastBuffer = buffers[buffers.length - 1];
    if (!lastBuffer) return allocBuffer();
    ...
    if (remainingVertices < requiredVertices || remainingIndices < requiredIndices) return allocBuffer();
    return lastBuffer;
}
function allocAtlas(buffer) {
    ...
    const shelf = new window.ShelfPack(ATLAS_SIZE, ATLAS_SIZE, {
        autoResize: false
    });
    const atlas = {
        texture,
...
function tryFindSpaceForTexture(atlas, width, height) {
    const bin = atlas.shelf.packOne(width, height);
    return bin;
}
function getSpaceInAtlas(buffer, width, height) {
...
const atlasGenerationQueue = new Map();
function addRenderToAtlasQueue(atlas, space, image) {
...
function processAtlasGenerationQueue() {
    if (atlasGenerationQueue.size < 1) return;
...
function debugOutputAtlas(atlas) {
...
function debugOutputAtlasPng(atlas) {
    const width = ATLAS_SIZE;
    const height = ATLAS_SIZE;
    gl.bindFramebuffer(gl.FRAMEBUFFER, atlas.frameBuffer);
    const canvas = document.createElement('canvas');
...
function compileShader(name, uniforms, attributes) {
    ...
    if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) throw "ERROR compiling vertex shader for " + name + "!\n" + gl.getShaderInfoLog(vertexShader);
    gl.compileShader(fragmentShader);
    if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) throw "ERROR compiling fragment shader for " + name + "!\n" + gl.getShaderInfoLog(fragmentShader);
    const program = gl.createProgram();
    gl.attachShader(program, vertexShader);
    gl.attachShader(program, fragmentShader);
    gl.linkProgram(program);
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) throw "ERROR linking program!\n" + gl.getProgramInfoLog(program);
    gl.validateProgram(program);
    if (!gl.getProgramParameter(program, gl.VALIDATE_STATUS)) throw "ERROR validating program!\n" + gl.getProgramInfoLog(program);
    const instance = {
        program: program,
        use: function() {
...
function glEnumToString(gl, value) {
    ...
    if (value === gl.NO_ERROR) return "NO_ERROR";
    for(const p in gl){
        if (gl[p] === value) return p;
    }
...
function create3DContextWithWrapperThatThrowsOnGLError(context) {
    ...
    for(const i in context)try {
        if (typeof context[i] === 'function') wrap[i] = createGLErrorWrapper(context, i);
        else wrap[i] = context[i];
    } catch (e) {
...
function controls_wheel(e) {
    ...
    if (isTouchPad) moveCameraBy(e.deltaX, e.deltaY);
    else scaleCameraBy(e.deltaY);
    e.preventDefault();
}
